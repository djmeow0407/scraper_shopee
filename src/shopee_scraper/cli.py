...
def main(
    ...
    chrome_version: Annotated[int | None, typer.Option(help="Ép major version của Chrome")] = None,
...
def _report(store: JsonStore) -> None:
...
def search(
    ...
    _report(scraper.run(review_limit=review_limit, mode=stars))
...
def categories(
    ...
    count = scrape_categories(settings, target)
    if count:
